...
<html>
  <head>
	...
	<script type="text/javascript" src="js/jquery.raty.js"></script>
     ...
     <link type="text/css" href="css/recommendMovie.css" rel="stylesheet" />	
    ...
    <script type="text/javascript">
		...
		function addStars(){
    		...
    		var movieId;
    		// 获取数据大小
    		var size = $("table[class='rMoviesTable'] input:hidden[name='size']").val();
    		// 给每个电影增加星级评分
    		for(var i=0; i<size;i++){
    			...
    			$("#" + movieId).raty({
				  ...
				  click: function(score){
				  	var url = "UserAction_evaRecommendMovie.action";
				  	//alert(url);
				  	...
				  	$.post(url, data, function(msg){
					  	...
					  	alert("评分成功！");
					  	$("table[class='rMoviesTable']").empty().append(msg);
  ...
  <body>
	...
	<table class="rMoviesTable">
	
	<s:if test="recommendMovieList != null && recommendMovieList.size() > 0">
		<s:iterator value="recommendMovieList" status="st">
			<tr>
			...
			<tr>
				...
				<td>
					<div id="<s:property value='"movieId-" + #st.index'/>"></div>
					<input type="hidden" value="<s:property value='id'/>"/>
					<input name="size" type="hidden" value="<s:property value='recommendMovieList.size()'/>"/>
				</td>
			</tr>
		</s:iterator> 
	</s:if>
